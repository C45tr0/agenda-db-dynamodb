...
var expect = require('expect.js'),
    ...
    Job = require( path.join('..', 'node_modules', 'agenda', 'lib', 'job.js') );

var sequelize = Support.sequelize;
// create agenda instances
var jobs = null;

function clearJobs(done) {
  jobs._dbAdapter._collection.destroy({truncate: true}).then(done).catch(done);
}

// Slow timeouts for travis
var jobTimeout = process.env.TRAVIS ? 1500 : 1000;
...
describe("agenda", function() {
  beforeEach(function(done) {
    jobs = new Agenda({
      ...
      db: { collection: "agenda-test" }
    }, function(err) {
      jobs._dbAdapter._collection.sync().then(function() {
  ...
  describe('Agenda', function() {
    ...
    describe('configuration methods', function() {
      it('sets up correctly with url', function() {
        var agenda = new Agenda({
          adapter: require('../index.js'),
          db: {
            credentials: {
              connectionString: Support.getTestUrl(),
              dialectOptions: {
                logging: false,
              },
            },
        ...
        expect(agenda._dbAdapter._collection.tableName).to.equal('agenda-test');
      });

      it('sets the _db directly when passed as an option', function() {
        var agenda = new Agenda({adapter: require('../index.js'), connection: sequelize, db: { collection: "agenda-test" }});
        expect(agenda._dbAdapter._collection.tableName).to.equal('agenda-test');
      });
    });
    ...
    describe('configuration methods', function() {
      ...
      describe('sequelize', function() {
        it('sets the collection correctly', function() {
          var agenda = new Agenda({adapter: require('../index.js'), db: { collection: "agenda-test" }});
          agenda.connection(sequelize);
          expect(agenda._dbAdapter._collection.tableName).to.equal('agenda-test');
        });

        it('returns itself', function() {
          var agenda = new Agenda({adapter: require('../index.js'), db: { collection: "agenda-test" }});
          expect(agenda.connection(sequelize)).to.be(agenda);
        });
      });
            ...
            jobs.create('unique job', {type: 'active', userId: '123', 'other': true}).unique({'data.type': 'active', 'data.userId': '123'}).schedule("now").save(function(err, job1) {
              setTimeout(function() { // Avoid timing condition where nextRunAt coincidentally is the same
                jobs.create('unique job', {type: 'active', userId: '123', 'other': false}).unique({'data.type': 'active', 'data.userId': '123'}).schedule("now").save(function(err, job2) {
                  ...
                  jobs._dbAdapter._collection.findAll({
                    where: {
                      name: 'unique job',
                    },
                  }).then(items => {
    ...
    describe('cancel', function() {
      ...
      afterEach(function(done) {
        jobs._dbAdapter._collection.destroy({where: {name: {$in: ['jobA', 'jobB']}}}).then(result => {
          done();
        }).catch(err => {
          done(err);
        });
  ...
  describe('Job', function() {
      ...
      it('removes the job', function(done) {
        ...
        job.save(function(err) {
          ...
          job.remove(function(err) {
            if(err) return done(err);
            jobs._dbAdapter._collection.findAll({where: {_id: job.attrs._id}}).then(j => {
              expect(j).to.have.length(0);
              done();
            }).catch(err => {
              done(err);
      ...
      it('clears locks on stop', function(done) {
        ...
        setTimeout(function() {
          jobs.stop(function(err, res) {
            jobs._dbAdapter._collection.findOne({name: 'longRunningJob'}).then(job => {
    ...
    describe("job lock", function(){
      ...
      it("runs a recurring job after a lock has expired", function(done) {
        ...
        jobs.define("lock job", {lockLifetime: 150}, function(job, cb){
          startCounter++;
        ...
        expect(jobs._definitions["lock job"].lockLifetime).to.be(150);
    ...
    describe("Integration Tests", function() {
      ...
      describe('.every()', function() {
        ...
        it('Should not rerun completed jobs after restart', function(done) {
          ...
          var startService = function() {
            ...
            var n = cp.fork( serverPath, [ Support.getTestUrl(), 'daily' ] );
        ...
        it('Should properly run jobs when defined via an array', function(done) {
          ...
          var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
          var n = cp.fork( serverPath, [ Support.getTestUrl(), 'daily-array' ] );
      ...
      describe('schedule()', function() {
        ...
        it('Should not run jobs scheduled in the future', function(done) {
          ...
          var startService = function() {
            var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
            var n = cp.fork( serverPath, [ Support.getTestUrl(), 'define-future-job' ] );
        ...
        it('Should run past due jobs when process starts', function(done) {
          ...
          var startService = function() {
            var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
            var n = cp.fork( serverPath, [ Support.getTestUrl(), 'define-past-due-job' ] );
        ...
        it('Should schedule using array of names', function(done) {
          ...
          var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
          var n = cp.fork( serverPath, [ Support.getTestUrl(), 'schedule-array' ] );
      ...
      describe('now()', function() {
        ...
        it('Should immediately run the job', function(done) {
          ...
          var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
          var n = cp.fork( serverPath, [ Support.getTestUrl(), 'now' ] );
