...
function clearJobs(done) {
  var createTables = function() {
    ...
    jobs._dbAdapter._vogels.createTables(function(err) {
      if (err) {
        throw new Error(err);
      } else {
        done();
  ...
  try {
    jobs._dbAdapter._collection.deleteTable(function(err) {
      createTables();
    });
  } catch(e) {
    createTables();
  ...
  describe('Agenda', function() {
    ...
    describe('configuration methods', function() {
      it('sets the _db directly when passed as an option', function() {
        var agenda = new Agenda({adapter: require('../index.js'), connection: vogels.dynamoDriver(), db: { collection: "agenda-test" }});
    ...
    describe('configuration methods', function() {
      ...
      describe('connection', function() {
        it('sets the _db directly', function() {
          ...
          agenda.connection(vogels.dynamoDriver());
          expect(agenda._dbAdapter._collection.tableName()).to.equal('agenda-test');
        });

        it('returns itself', function() {
          var agenda = new Agenda({adapter: require('../index.js')});
          expect(agenda.connection(vogels.dynamoDriver())).to.be(agenda);
        });
      });
        ...
        setTimeout(function() {
          jobs.stop(function(err, res) {
            jobs._dbAdapter._collection.findOne({name: 'longRunningJob'}, function(err, job) {
              expect(job.lockedAt).to.eql(new Date(1));
              done();
            });
          });
        }, jobTimeout);
    ...
    describe("Integration Tests", function() {
      ...
      describe('.every()', function() {
        ...
        it('Should not rerun completed jobs after restart', function(done) {
          ...
          var startService = function() {
            ...
            var n = cp.fork( serverPath, [ dynamoConfig, 'daily' ] );
        ...
        it('Should properly run jobs when defined via an array', function(done) {
          ...
          var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
          var n = cp.fork( serverPath, [ dynamoConfig, 'daily-array' ] );
      ...
      describe('schedule()', function() {
        ...
        it('Should not run jobs scheduled in the future', function(done) {
          ...
          var startService = function() {
            var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
            var n = cp.fork( serverPath, [ dynamoConfig, 'define-future-job' ] );
        ...
        it('Should run past due jobs when process starts', function(done) {
          ...
          var startService = function() {
            var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
            var n = cp.fork( serverPath, [ dynamoConfig, 'define-past-due-job' ] );
        ...
        it('Should schedule using array of names', function(done) {
          ...
          var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
          var n = cp.fork( serverPath, [ dynamoConfig, 'schedule-array' ] );
      ...
      describe('now()', function() {
        ...
        it('Should immediately run the job', function(done) {
          ...
          var serverPath = path.join( __dirname, 'fixtures', 'agenda-instance.js' );
          var n = cp.fork( serverPath, [ dynamoConfig, 'now' ] );
