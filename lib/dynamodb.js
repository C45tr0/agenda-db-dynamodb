...
function getCollection(tableName) {
  return dynogels.define('Job', {
    hashKey : 'name',
    rangeKey: '_id',
    timestamps : true,
    schema : {
      name : Joi.string(),
      _id : dynogels.types.uuid(),
      type : Joi.string(),
...
DynamoDBAdapter.prototype.database = function(credentials, collection, cb) {
  if (typeof credentials !== "object") {
    this._dynogels.AWS.config.loadFromPath(credentials);
  } else {
    this._dynogels.AWS.config.update(credentials);
  }
...
DynamoDBAdapter.prototype.db_init = function(cb ){
  ...
  this._dynogels.createTables(function(err) {
...
function cleanDates(obj) {
  ...
  keys.forEach(function(key) {
    if ([
      ...
      'lastFinishedAt',
      'lockedAt'
    ].indexOf(key) != -1 && obj[key] instanceof Date) {
      obj[key] = obj[key]/1;
    }
  });
...
DynamoDBAdapter.prototype._findAndLockNextJob = function(jobName, nextScanAt, lockDeadline, cb) {
  ...
  this._collection.query(jobName).loadAll()
    .usingIndex('NamePriorityIndex').ascending()
